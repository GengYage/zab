...
public class DataManager {
    ...
    private final static byte[] LAST_INDEX_KEY = "last_index_key".getBytes(StandardCharsets.UTF_8);
    ...
    public boolean put(final String key, final String value) {
        try {
            if (logLock.writeLock().tryLock(3000, TimeUnit.MILLISECONDS)) {
                logDb.put(key.getBytes(StandardCharsets.UTF_8), JSONSerializable.encode(value));
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public String get(final String key) {
        try {
            if (logLock.readLock().tryLock(3000, TimeUnit.MILLISECONDS)) {
                byte[] value = logDb.get(key.getBytes(StandardCharsets.UTF_8));
                if (value == null) {
                    return null;
                }
    ...
    public boolean write(final Data data) {
        try {
            if (committedLock.writeLock().tryLock(3000, TimeUnit.MILLISECONDS)) {
                ...
                committedDb.put(Long.toString(nextIndex).getBytes(StandardCharsets.UTF_8), JSONSerializable.encode(data));
    ...
    public Data read(Long index) {
        try {
            if (committedLock.readLock().tryLock(3000, TimeUnit.MILLISECONDS)) {
                byte[] value = committedDb.get(index.toString().getBytes(StandardCharsets.UTF_8));
                if (value != null) {
    ...
    public void removeFromIndex(Long index) {
        try {
            if (committedLock.writeLock().tryLock(3000, TimeUnit.MILLISECONDS)) {
                byte[] startIndex = index.toString().getBytes(StandardCharsets.UTF_8);
                byte[] lastIndex = getLastIndex().toString().getBytes(StandardCharsets.UTF_8);
    ...
    public Long getLastIndex() {
        try {
            if (committedLock.readLock().tryLock(3000, TimeUnit.MILLISECONDS)) {
                ...
                if (lastIndex != null) {
                    return Long.valueOf(new String(lastIndex, StandardCharsets.UTF_8));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void updateLastIndex(final Long index) {
        try {
            committedDb.put(LAST_INDEX_KEY, index.toString().getBytes(StandardCharsets.UTF_8));
